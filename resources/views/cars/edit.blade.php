                ...
                <div class="mb-3">
                    ...
                    <select id="color_list_id" class="form-select" name="color_list_id">
                        ...
                        @foreach($colorLists as $colorList)
                            <option value="{{$colorList->id}}" {{$model->colorList->id == $colorList->id?'selected':''}} style="background-color: {{$colorList->code}}; color:{{strtolower($colorList->name)=='white'?'black':'white'}}">{{$colorList->name}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
